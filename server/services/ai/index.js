...
export const generateUrduGrammarQnA = asyncHandler(async(req,res)=>{
  // const {prompt} = req.body;
  let content;
  try {
    const prompt = `
Create a  JSON object for an Urdu grammar quiz with this structure, it should have 5 questions:

{
  "question": ["sentence1", "sentence2", "sentence3", "sentence4"],
  "correct": "correct_sentence"
}

Rules:
1. "question" array must have 4 short Urdu sentences.
2. Exactly 1 sentence is grammatically correct; the other 3 must contain grammatical errors.
3. "correct" must contain the grammatically correct sentence from the array.
4. Do not include any explanations, instructions, or text outside the JSON.
5. Keep sentences simple and suitable for a beginner-intermediate Urdu quiz.
`;
     ...
     if(!content){
      ...
      success : false,
      statusCode : STATUS_CODES.INTERNAL_SERVER_ERROR,
      message : "Failed to generate Urdu grammar questions"
    })
    }
  ...
  let rawContent = content; // string with ```json
  // strip markdown
  rawContent = rawContent.replace(/```json|```/g, "").trim();
  const quizObject = JSON.parse(rawContent);
